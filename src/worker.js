  ...
  async applyUpdate(roomId, update) {
      ...
      if (update.action === 'start_game') {
          ...
          if (update.gameMode === 'turn') {
              ...
              if (Array.isArray(update.players) && update.players.length > 0) {
                  ...
                  doPlayers.forEach(p => merged.set(p.id || p, p));
                  ...
                  mergedPlayers = mergedPlayers.filter(p => !eliminatedSet.has(p.id || p));

                  const before = state.players?.length || 0;
              ...
              state.eliminatedPlayers = [];
              console.log(`[start_game] 🔍 players 최종: ${state.players.length}명`);
      ...
      if (update.action === 'new_game') {
          ...
          if (update.gameMode === 'turn' || state.gameMode === 'turn') {
              ...
              // KV의 players (현재 접속 중)를 기준으로, DO+KV를 합친 뒤 활성만 남김
              if (Array.isArray(update.players) && update.players.length > 0) {
                  const activePlayerIds = new Set(update.players.map(p => p.id || p));
                  const doPlayers = Array.isArray(state.players) ? [...state.players] : [];
                  const kvPlayers = update.players;

                  // DO + KV 합치기 (id 기준)
                  const mergedMap = new Map();
                  doPlayers.forEach(p => mergedMap.set(p.id || p, p));
                  kvPlayers.forEach(p => {
                      const pid = p.id || p;
                  ...
                  // 활성(KV에 존재)만 남기기
                  const mergedPlayers = Array.from(mergedMap.values()).filter(p => activePlayerIds.has(p.id || p));
                  const beforeCount = state.players?.length || 0;
                  state.players = mergedPlayers;
                  const removedCount = beforeCount - state.players.length;
                  if (removedCount > 0) {
              ...
              // 턴제는 lastSeen 필터 미적용 (정상 접속자 오인 방지)
              } else {
              ...
              // KV의 players를 기준으로 DO+KV를 합친 뒤 활성만 남김 (시간제도 동일하게 복구)
              if (Array.isArray(update.players) && update.players.length > 0) {
                  const activePlayerIds = new Set(update.players.map(p => p.id || p));
                  const doPlayers = Array.isArray(state.players) ? [...state.players] : [];
                  const kvPlayers = update.players;
      ...
      if (update.action === 'submit_word' && state.gameMode === 'turn') {
          ...
          if (isValid) {
              ...
              // 단어 맞추면 연속 첫턴탈락 카운트 리셋
              if (state.consecutiveFirstTurnFails?.[playerId]) {
                  state.consecutiveFirstTurnFails[playerId] = 0;
              }
      ...
      if (update.action === 'turn_timeout' && state.gameMode === 'turn') {
          ...
          if (playerId === state.currentTurnPlayerId) {
              ...
              if (state.playerLives[playerId] < 0) {
                  ...
                  // 🚀 첫턴 탈락 체크 (turnCount가 0이면 한 번도 단어 못 맞추고 탈락)
                  const playerTurnCount = state.turnCount?.[playerId] || 0;
                  if (playerTurnCount === 0) {
                      if (!state.consecutiveFirstTurnFails) state.consecutiveFirstTurnFails = {};
                      state.consecutiveFirstTurnFails[playerId] = (state.consecutiveFirstTurnFails[playerId] || 0) + 1;
                      console.log(`[턴제] ${playerId} 첫턴 탈락! 연속 ${state.consecutiveFirstTurnFails[playerId]}회`);
                      
                      // 3회 연속 첫턴 탈락 → remove_player로 강퇴
                      if (state.consecutiveFirstTurnFails[playerId] >= 3) {
                          const playerObj = (state.players || []).find(p => (p.id || p) === playerId);
                          const playerName = playerObj?.name || playerId;
                          console.log(`[턴제] 🚨 ${playerName} 첫턴 3회 연속 탈락 - 강퇴!`);
                          
                          // 채팅 메시지
                          if (!state.chatMessages) state.chatMessages = [];
                          state.chatMessages.push({ sender: '시스템', message: `${playerName}님이 무응답으로 퇴장되었습니다.`, timestamp: now });
                          
                          // remove_player 액션 호출
                          await this.applyUpdate(roomId, { action: 'remove_player', playerId: playerId });
                          return state;
                      }
                  } else {
                      // 첫턴 아닌 탈락 → 카운트 리셋
...
async function handleJoinRoom(request, env) {
  ...
  if (!existingPlayer) {
      ...
      // 🚀 턴제: 새 플레이어를 DO에도 즉시 동기화 (게임 중/대기실 공통)
      if (roomData.gameMode === 'turn' && env.GAME_STATE) {
          try {
              const id = env.GAME_STATE.idFromName(roomId);
              const stub = env.GAME_STATE.get(id);
              ...
              console.log(`[join-room] 🔍 턴제 새 플레이어 DO 동기화 완료 (${roomData.players.length}명)`);
          } catch (e) {
              console.error('[join-room] 턴제 새 플레이어 DO 동기화 실패 (무시):', e);
          }
      }
  } else {
...
async function handleLeaveRoom(request, env) {
  ...
  let finalRemainingPlayers = 0; // 최종 남은 플레이어 수
  let finalNewHostId = null; // 최종 새 방장 ID
  ...
  if (roomData.gameMode === 'turn' && env.GAME_STATE) {
      try {
          ...
          if (removeResponse.ok) {
              ...
              // DO 처리가 성공했다면, KV는 DO로부터 이미 최신화되었을 가능성이 높음.
              // 하지만 혹시 모를 상황에 대비하여 KV에서 다시 최신 roomData를 가져와 판단
              const latestRoomData = await env.ROOM_LIST.get(roomId, 'json');
              const currentPlayersInKV = latestRoomData?.players || [];
              finalRemainingPlayers = currentPlayersInKV.length;
              finalNewHostId = latestRoomData?.hostId || null;
              
              const shouldDeleteTurnRoom = currentPlayersInKV.length <= 1; 
              
              if (shouldDeleteTurnRoom) { // 턴제 모드에서 방 삭제가 필요한 경우
                  try {
                      await env.ROOM_LIST.delete(roomId);
                      try {
                          const recentRooms = await env.ROOM_LIST.get('_recent_rooms', 'json') || [];
              ...
              } else { // 🚀 수정: 턴제 모드에서 방이 삭제되지 않는 경우 KV 업데이트
                  await env.ROOM_LIST.put(roomId, JSON.stringify(latestRoomData), { // 🚀 latestRoomData 사용
                      metadata: {
                          id: roomId,
                          roomNumber: latestRoomData.roomNumber || 0,
                          createdAt: latestRoomData.createdAt,
                          playerCount: currentPlayersInKV.length, // DO로부터 가져온 최신 KV 상태 반영
                          gameStarted: latestRoomData.gameStarted || false,
                          roundNumber: latestRoomData.roundNumber || 0,
                          title: latestRoomData.title || '초성 배틀방',
                          gameMode: latestRoomData.gameMode || 'turn' // 턴제 모드
                      }
                  });
              }
          }
      } catch (e) {
          console.error(`[leave-room] 턴제 모드: DO에서 플레이어 ${playerId} 제거 실패 (예외 발생):`, e);
          // doResponseOk는 false. KV 업데이트를 시도하지 않음.
      }
  } 
  // 🚀 시간제 모드 처리 (DO를 사용하지 않음)
  else if (roomData.gameMode === 'time') { 
      ...
      finalRemainingPlayers = roomData.players.length;
      finalNewHostId = roomData.hostId || null;
  ...
  return jsonResponse({ 
      success: true, 
      remainingPlayers: finalRemainingPlayers,
      newHostId: finalNewHostId
  });
}
